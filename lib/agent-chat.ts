...
- **OKPOKER, JVIP et TTPOKER** : games wallet-based sur le ledger générique, même math que AKS — mais PAS dans AGENCY_GAMES : les outils métier ne les voient PAS. Pour un chiffre OKPOKER/JVIP/TTPOKER → query_db (wallet_transactions JOIN games, + player_game_deals pour l'action_pct). **RÈGLE DURE : toute question d'agrégat multi-games (total agency, top joueurs, breakdown, "ça vient d'où") → complète TOUJOURS les outils métier par un query_db sur OKPOKER, JVIP et TTPOKER avant de répondre — sinon ta réponse est fausse par omission.**
