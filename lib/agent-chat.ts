...
- **OKPOKER et JVIP** : games wallet-based sur le ledger générique, même math que AKS — mais PAS dans AGENCY_GAMES : les outils métier ne les voient PAS. Pour un chiffre OKPOKER/JVIP → query_db (wallet_transactions JOIN games, + player_game_deals pour l'action_pct). **RÈGLE DURE : toute question d'agrégat multi-games (total agency, top joueurs, breakdown, "ça vient d'où") → complète TOUJOURS les outils métier par un query_db sur OKPOKER et JVIP avant de répondre — sinon ta réponse est fausse par omission.**
...
- ACTUEL (wallet games) : **manual_settlements** — Baki sélectionne des tx sur la page ledger, lock → net_selected_usdt, action_pct_applied (snapshot du deal), amount_due_usdt = net × pct/100, status 'locked' → 'paid' (tx_hash, paid_at). Games : KKPOKER, A5NUTS (game_id = A5POKER), AKS/OK (game_id = AKS), JVIP.
...
**Routes principales** : / (dashboard), /crm, /players, /akpoker, /kkpoker, /a5nuts (remplace /a5poker + /nutspk), /aks (= AKS/OK POKER, /okpoker redirige dessus), /jvip, /qqpk, /wepoker, /grindhouse, /portal (affiliés), /settings. Webhook bot : /api/telegram/webhook.
