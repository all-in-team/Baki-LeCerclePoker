...
- **NEXAPOKER — même table, AUTRE mécanique. Ne JAMAIS lui appliquer la formule ci-dessus.** Ce qui s'y règle est la PART D'ACTION, et son assiette est le **win/loss saisi à la main** par Baki (table nexa_player_weekly_winloss), pas une sélection de transactions. Une ligne NEXA porte kind='action', **zéro transaction back-linkée**, net_selected_usdt = Σ des win/loss des semaines couvertes (et non retraits − dépôts), et amount_due_usdt = Σ (win/loss × action_pct/100 de la semaine). action_pct_applied vaut **0 quand les taux diffèrent** d'une semaine à l'autre sur la sélection — dans ce cas ne jamais écrire « net × 0% », le détail par semaine est dans nexa_action_settlement_weeks. Les buy-ins et cash-outs NEXA sont de la **trésorerie** : ils ne sont jamais réglés, jamais flaggés settled, et NEXAPOKER est volontairement exclu de la détection d'impayés (rien ne pourrait les éteindre). Le rakeback NEXA **a désormais son propre flux de règlement**, distinct de la part d'action : lignes kind='rakeback' dans la même table, **amount_due_usdt NÉGATIF** (le rakeback SORT — c'est le signe, jamais la colonne kind, qui est compté par les agrégats), net_selected_usdt = ASSIETTE totale et non un net de transactions, et **le produit assiette × taux ne retombe PAS sur le montant** dès qu'il y a du makeup : ne jamais écrire « net × action % » sur une ligne de rakeback, le détail par semaine est dans nexa_rakeback_settlement_weeks. La plage réglée est contiguë et part de la plus ancienne semaine non réglée (le makeup se chaîne). Une semaine dont le contrôle a échoué PLAFONNE la plage ; les lignes kind='rakeback_ack' à montant 0 sont l'acte par lequel Baki acte un tel écart à 0 pour débloquer la suite — ce ne sont **pas des paiements**, elles ne bougent aucun argent. Régler le rakeback ne solde PAS le makeup restant et ne modifie aucun montant à venir. Son % vit dans nexa_player_rakeback et settings.nexa_default_rakeback_pct : va lire la valeur avant de l'affirmer, c'est un ÉTAT de la donnée et non une règle.
