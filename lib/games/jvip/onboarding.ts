...
function getJvipGameId(): number | null {
  const row = getDb().prepare(`SELECT id FROM games WHERE name = ?`).get(JVIP_GAME_NAME) as { id: number } | undefined;
  return row?.id ?? null;
}

function getJvipDeal(playerId: number): { action_pct: number; rakeback_pct: number } | null {
...
// JVIP pitch — action % is MODULABLE, chosen by the owner at /startjvip launch
// (shared free-text prompt) and passed in here. The deal is upserted (re-running
// /startjvip with a new % updates it). Deliberately simpler than the AKS pitch:
// the % action is stated clearly, that's it (demande Baki).
export async function sendJvipPitch(
  chatId: number,
  playerId: number,
  ...
  setSession(chatId, "jvip_pitch_sent" as Step, playerId, player.telegram_id);
  ...
  await sendMsg(chatId,
    `${tag}\n\n🃏 <b>${player.name}</b> — on te propose JVIP !\n\n` +
    `Le deal est simple :\n\n` +
  ...
  // un clic explicite sur "J'accepte" (handleJvipCallback → jvip_accept).
...
export async function handleJvipCallback(
  ...
  if (data === "jvip_choice_question") {
    ...
    await sendMsg(AGENT_CHAT_ID,
      `💬 <b>Question JVIP onboarding — ${playerName}</b>\n` +
  ...
  if (data === "jvip_accept") {
    if (session.step !== ("jvip_pitch_sent" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});

    const gameId = getJvipGameId();
    const deal = player ? getJvipDeal(player.id) : null;
    ...
    setSession(chatId, "jvip_wallet_check" as Step, session.player_id, session.expected_tg_id);

    await sendMsg(chatId, `✅ <b>Deal accepté !</b>`, tid);
    if (JVIP_GAME_LINK) {
      await sleep(1200);
      // Link revealed ONLY now — after explicit acceptance.
      await sendMsg(chatId, `🃏 Parfait ! Rejoins la game JVIP 👉 ${JVIP_GAME_LINK}`, tid);
    }
    await sleep(1500);
    await sendMsgKeyboard(chatId,
      `Avant de finaliser, question rapide : tu as déjà un wallet crypto USDT en TRC20 (réseau Tron) ?`,
      [
        [{ text: "✅ Oui j'ai un wallet", callback_data: "jvip_wallet_yes" }],
        [{ text: "❌ Non, j'en ai pas", callback_data: "jvip_wallet_no" }],
      ],
      tid
    );

    await sendMsg(AGENT_CHAT_ID,
      `🎉 <b>Deal accepté JVIP — ${playerName}</b>\n` +
  ...
  if (data === "jvip_wallet_yes") {
    if (session.step !== ("jvip_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
    ...
    setSession(chatId, "awaiting_jvip_cashout_wallet" as Step, session.player_id, session.expected_tg_id);
    return;
  }

  // ── Wallet NO → tutorial + bail ──
  if (data === "jvip_wallet_no") {
    if (session.step !== ("jvip_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
...
// ── Raw message handler for JVIP: wallet collection states ──

export async function handleJvipRawMessage(
  text: string,
  chatId: number,
  ...
  if (session.step === ("awaiting_jvip_cashout_wallet" as Step)) {
    ...
    const gameId = getJvipGameId();
    if (!gameId) {
      await reply(`❌ Erreur interne (game JVIP introuvable). Contacte @baki77777`);
      return true;
    }
    ...
    setSession(chatId, "awaiting_jvip_game_wallet" as Step, session.player_id, session.expected_tg_id, text);

    await reply(`✅ Adresse de retrait enregistrée.`);
    await sleep(1500);
    await reply(
      `<b>Étape 2 — L'adresse de dépôt JVIP</b>\n\n` +
      `Ouvre l'app JVIP, va sur ton profil et clique sur "Deposit" (ou 充币). ` +
  ...
  if (session.step === ("awaiting_jvip_game_wallet" as Step)) {
    if (!TRC20_RE.test(text)) {
      await reply(`❌ Format incorrect. Une adresse TRON commence par T et fait 34 caractères. Réessaie.`);
      return true;
    }
    ...
    if (text === cashoutAddress) {
      await reply(
        ...
        `L'adresse game est fournie par JVIP (bouton Deposit dans l'app), pas par ta wallet perso. Réessaie.`
      );
      return true;
    }
    ...
    const deal = getJvipDeal(session.player_id);
    const actionPct = deal?.action_pct ?? JVIP_DEFAULT_ACTION_PCT;
    const playerPct = 100 - actionPct;

    await reply(
      `✅ Adresse game JVIP enregistrée.\n\n` +
      `🎯 <b>Setup complet !</b>\n` +
    ...
    // chatId means the flow ran via the ?start=jvip deep link, not a linked group.
    const viaDeepLink = chatId > 0;
    await sendMsg(AGENT_CHAT_ID,
      (viaDeepLink
        ? `🎉 <b>${playerName} onboardé JVIP via deep link</b> — % actuel : ${actionPct} (ajuste si besoin)\n`
        : `🎉 <b>Onboarding JVIP complet — ${playerName}</b>\n`) +
      `Deal : ${playerPct}/${actionPct} (action_pct=${actionPct})\n` +
      `Wallet retrait : <code>${cashoutAddress}</code>\n` +
      `Wallet game JVIP : <code>${text}</code>` +
      (JVIP_GAME_LINK ? `\nLien : ${JVIP_GAME_LINK}` : "")
    );
