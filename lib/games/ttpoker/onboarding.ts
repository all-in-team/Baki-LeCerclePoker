...
function getTtpokerGameId(): number | null {
  const row = getDb().prepare(`SELECT id FROM games WHERE name = ?`).get(TTPOKER_GAME_NAME) as { id: number } | undefined;
  return row?.id ?? null;
}

function getTtpokerDeal(playerId: number): { action_pct: number; rakeback_pct: number } | null {
  const gameId = getTtpokerGameId();
...
// TTPOKER pitch — action % is MODULABLE, chosen by the owner at /startttpoker launch
// (shared free-text prompt) and passed in here. The deal is upserted (re-running
// /startttpoker with a new % updates it). Deliberately simpler than the AKS pitch:
// the % action is stated clearly, that's it (demande Baki).
export async function sendTtpokerPitch(
  chatId: number,
  playerId: number,
  ...
  setSession(chatId, "ttpoker_pitch_sent" as Step, playerId, player.telegram_id);
  ...
  await sendMsg(chatId,
    `${tag}\n\n🃏 <b>${player.name}</b> — on te propose TTPOKER !\n\n` +
    `Le deal est simple :\n\n` +
  ...
  // un clic explicite sur "J'accepte" (handleTtpokerCallback → ttpoker_accept).
  await sendMsgKeyboard(chatId, `Tu valides le deal ?`, [
    [{ text: "✅ J'accepte le deal", callback_data: "ttpoker_accept" }],
    [{ text: "❓ J'ai une question", callback_data: "ttpoker_choice_question" }],
  ], tid);
}

// ── Deep-link DM flow: https://t.me/LeCercle_Lebot?start=ttpoker ──
// The player NEVER chooses his % here (règle métier) : existing TTPOKER deal is kept
...
export async function handleTtpokerDeepLink(
  chatId: number,
  ...
  const gameId = getTtpokerGameId();
  if (!gameId) {
    await sendMsg(chatId, `❌ Erreur interne (game TTPOKER introuvable). Contacte @baki77777`);
    return;
  }
  ...
  if (player) {
    await startTtpokerDmFlow(chatId, player, gameId);
    return;
  }
  ...
  setSession(chatId, "ttpoker_dm_waiting_name" as Step, null, from.id, from.username ?? null);
  await sendMsg(chatId,
    `🃏 <b>Bienvenue sur Le Cercle !</b>\n\n` +
    `Avant de te présenter le deal TTPOKER, une question rapide :\n` +
    `c'est quoi ton pseudo (ou prénom) ?`
  );
}

async function startTtpokerDmFlow(
  chatId: number,
  ...
  const existingDeal = getTtpokerDeal(player.id);
  ...
  if (existingDeal && hasWallets) {
    await sendMsg(chatId,
      `✅ <b>${player.name}</b>, TTPOKER est déjà configuré pour toi !\n\n` +
      `Questions ? → @baki77777`
    );
    return;
  }

  // Existing deal kept as-is; otherwise the game's default. sendTtpokerPitch upserts
  ...
  const defaultPct = (db.prepare(`SELECT default_action_pct FROM games WHERE id = ?`).get(gameId) as { default_action_pct: number | null } | undefined)?.default_action_pct
    ?? TTPOKER_DEFAULT_ACTION_PCT;
  const pct = existingDeal?.action_pct ?? defaultPct;

  await sendTtpokerPitch(chatId, player.id, player, pct, undefined);
}
...
export async function handleTtpokerDmNameRawMessage(
  text: string,
  chatId: number,
  session: { step: Step; player_id: number | null; expected_tg_id: number | null; pending_cmd?: string | null },
): Promise<boolean> {
  if (session.step !== ("ttpoker_dm_waiting_name" as Step)) return false;
...
export async function handleTtpokerCallback(
  ...
  // ── Question ──
  if (data === "ttpoker_choice_question") {
    ...
    await sendMsg(AGENT_CHAT_ID,
      `💬 <b>Question TTPOKER onboarding — ${playerName}</b>\n` +
  ...
  if (data === "ttpoker_accept") {
    ...
    const gameId = getTtpokerGameId();
    const deal = player ? getTtpokerDeal(player.id) : null;
    ...
    await sendMsgKeyboard(chatId,
      ...
      [
        [{ text: "✅ Oui j'ai un wallet", callback_data: "ttpoker_wallet_yes" }],
        [{ text: "❌ Non, j'en ai pas", callback_data: "ttpoker_wallet_no" }],
      ],
      tid
    );

    await sendMsg(AGENT_CHAT_ID,
      `🎉 <b>Deal accepté TTPOKER — ${playerName}</b>\n` +
  ...
  if (data === "ttpoker_wallet_yes") {
    if (session.step !== ("ttpoker_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
    ...
    setSession(chatId, "awaiting_ttpoker_cashout_wallet" as Step, session.player_id, session.expected_tg_id);
    return;
  }

  // ── Wallet NO → tutorial + bail ──
  if (data === "ttpoker_wallet_no") {
    if (session.step !== ("ttpoker_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
...
// ── Raw message handler for TTPOKER: wallet collection states ──

export async function handleTtpokerRawMessage(
  text: string,
  chatId: number,
  ...
  if (session.step === ("awaiting_ttpoker_cashout_wallet" as Step)) {
    ...
    const gameId = getTtpokerGameId();
    if (!gameId) {
      await reply(`❌ Erreur interne (game TTPOKER introuvable). Contacte @baki77777`);
      return true;
    }
    ...
    setSession(chatId, "awaiting_ttpoker_game_wallet" as Step, session.player_id, session.expected_tg_id, text);

    await reply(`✅ Adresse de retrait enregistrée.`);
    await sleep(1500);
    await reply(
      `<b>Étape 2 — L'adresse de dépôt TTPOKER</b>\n\n` +
      `Ouvre l'app TTPOKER, va sur ton profil et clique sur "Deposit" (ou 充币). ` +
  ...
  if (session.step === ("awaiting_ttpoker_game_wallet" as Step)) {
    if (!TRC20_RE.test(text)) {
      await reply(`❌ Format incorrect. Une adresse TRON commence par T et fait 34 caractères. Réessaie.`);
      return true;
    }
    ...
    if (text === cashoutAddress) {
      await reply(
        ...
        `L'adresse game est fournie par TTPOKER (bouton Deposit dans l'app), pas par ta wallet perso. Réessaie.`
      );
      return true;
    }
    ...
    const deal = getTtpokerDeal(session.player_id);
    const actionPct = deal?.action_pct ?? TTPOKER_DEFAULT_ACTION_PCT;
    const playerPct = 100 - actionPct;

    await reply(
      `✅ Adresse game TTPOKER enregistrée.\n\n` +
      `🎯 <b>Setup complet !</b>\n` +
    ...
    // chatId means the flow ran via the ?start=ttpoker deep link, not a linked group.
    const viaDeepLink = chatId > 0;
    await sendMsg(AGENT_CHAT_ID,
      (viaDeepLink
        ? `🎉 <b>${playerName} onboardé TTPOKER via deep link</b> — % actuel : ${actionPct} (ajuste si besoin)\n`
