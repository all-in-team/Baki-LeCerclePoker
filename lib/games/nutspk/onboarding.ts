...
import { NUTSPK_GAME_NAME, NUTSPK_GAME_LINK } from "./config";

const sleep = (ms: number) => new Promise(r => setTimeout(r, ms));

function getNutspkGameId(): number | null {
  const row = getDb().prepare(`SELECT id FROM games WHERE name = ?`).get(NUTSPK_GAME_NAME) as { id: number } | undefined;
  return row?.id ?? null;
}

function getNutspkDeal(playerId: number): { action_pct: number; rakeback_pct: number } | null {
  const gameId = getNutspkGameId();
...
export async function handleNutspkCallback(
  ...
  // ── Question ──
  if (data === "nutspk_choice_question") {
    ...
    await sendMsg(AGENT_CHAT_ID,
      `💬 <b>Question NUTSPK onboarding — ${playerName}</b>\n` +
  ...
  // This is the anti-bypass gate: the game link is sent ONLY here, after an
  // explicit acceptance click, never in the pitch.
  if (data === "nutspk_accept") {
    if (session.step !== ("nutspk_pitch_sent" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});

    const gameId = getNutspkGameId();
    const deal = player ? getNutspkDeal(player.id) : null;
    ...
    setSession(chatId, "nutspk_wallet_check" as Step, session.player_id, session.expected_tg_id);

    await sendMsg(chatId, `✅ <b>Deal accepté !</b>`, tid);
    await sleep(1200);
    // Link revealed ONLY now — after explicit acceptance.
    await sendMsg(chatId, `🃏 Parfait ! Rejoins la game NUTSPK 👉 ${NUTSPK_GAME_LINK}`, tid);
    await sleep(1500);
    await sendMsgKeyboard(chatId,
      `Avant de finaliser, question rapide : tu as déjà un wallet crypto USDT en TRC20 (réseau Tron) ?`,
      [
        [{ text: "✅ Oui j'ai un wallet", callback_data: "nutspk_wallet_yes" }],
        [{ text: "❌ Non, j'en ai pas", callback_data: "nutspk_wallet_no" }],
      ],
      tid
    );

    await sendMsg(AGENT_CHAT_ID,
      `🎉 <b>Deal accepté NUTSPK — ${playerName}</b>\n` +
  ...
  if (data === "nutspk_wallet_yes") {
    if (session.step !== ("nutspk_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
    ...
    setSession(chatId, "awaiting_nutspk_cashout_wallet" as Step, session.player_id, session.expected_tg_id);
    return;
  }

  // ── Wallet NO → tutorial + bail ──
  if (data === "nutspk_wallet_no") {
    if (session.step !== ("nutspk_wallet_check" as Step)) return;
    if (messageId) await editMessageReplyMarkup(chatId, messageId).catch(() => {});
...
// ── Raw message handler for NUTSPK: wallet collection states ──

export async function handleNutspkRawMessage(
  text: string,
  chatId: number,
  ...
  if (session.step === ("awaiting_nutspk_cashout_wallet" as Step)) {
    ...
    const gameId = getNutspkGameId();
    if (!gameId) {
      await reply(`❌ Erreur interne (game NUTSPK introuvable). Contacte @baki77777`);
      return true;
    }
    ...
    setSession(chatId, "awaiting_nutspk_game_wallet" as Step, session.player_id, session.expected_tg_id, text);

    await reply(`✅ Adresse de retrait enregistrée.`);
    await sleep(1500);
    await reply(
      `<b>Étape 2 — L'adresse de dépôt NUTSPK</b>\n\n` +
      `Ouvre l'app NUTSPK, va sur ton profil et clique sur "Deposit" (ou 充币). ` +
  ...
  if (session.step === ("awaiting_nutspk_game_wallet" as Step)) {
    if (!TRC20_RE.test(text)) {
      await reply(`❌ Format incorrect. Une adresse TRON commence par T et fait 34 caractères. Réessaie.`);
      return true;
    }
    ...
    if (text === cashoutAddress) {
      await reply(
        ...
        `L'adresse game est fournie par NUTSPK (bouton Deposit dans l'app), pas par ta wallet perso. Réessaie.`
      );
      return true;
    }
    ...
    const deal = getNutspkDeal(session.player_id);
    const actionPct = deal?.action_pct ?? 30;
    const playerPct = 100 - actionPct;

    await reply(
      `✅ Adresse game NUTSPK enregistrée.\n\n` +
      `🎯 <b>Setup complet !</b>\n` +
