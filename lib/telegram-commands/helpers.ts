...
export async function handleRawMessage(text: string, chatId: number, messageThreadId?: number) {
  ...
  // AKS custom % + wallet collection states — delegate to AKS module
  if (session.step === "waiting_aks_pct" || session.step === "awaiting_aks_cashout_wallet" || session.step === "awaiting_aks_game_wallet") {
