...
export default async function AKSPage({ searchParams }: { searchParams: Promise<{ filter?: string; player?: string }> }) {
  ...
  const filters = { game_name: "AKS" as const, since_date: startDate, end_date: endDate };
  ...
  let kpis = getLockAwareKPIsWithExtras(filters, "aks");
  ...
  const games = (getGames() as any[]).filter((g) => g.name === "AKS");
  const aksGameId = (getDb().prepare(`SELECT id FROM games WHERE name = 'AKS'`).get() as { id: number } | undefined)?.id;
  const walletMeres = aksGameId ? getWalletMeresForGame(aksGameId) : [];
  ...
  for (const p of players) {
    cashoutsByPlayer[p.id] = aksGameId ? getPlayerCashouts(p.id, aksGameId) : [];
    gameWalletsByPlayer[p.id] = aksGameId ? getPlayerGameWallets(p.id, aksGameId) : [];
  }
