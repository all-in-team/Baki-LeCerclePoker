...
export default async function AKSSettlementsPage({ searchParams }: { searchParams: Promise<{ week?: string; player?: string }> }) {
  ...
  return (
    <>
      <PageHeader title="AKS — Settlements" subtitle="Validation hebdomadaire des P&L wallet par joueur" />
      {filterPlayerName && (
        <div style={{ padding: "0 28px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="/aks/settlements" style={{ fontSize: 11, color: "var(--text-muted)", textDecoration: "none" }}>✕ Retirer le filtre</a>
        </div>
      )}
      <SettlementsClient
        weekStart={weekStart}
