...
export default function CRMPage() {
  ...
  const games = db.prepare(`
    SELECT pgd.player_id, GROUP_CONCAT(CASE WHEN LOWER(g.name) = 'tele' THEN 'AK' ELSE 'WP' END, ' ') AS badges
  ...
  games.forEach((g: any) => gamesByPlayer.set(g.player_id, g.badges));
              ...
              return (
                <tr key={p.id} style={{ borderBottom: "1px solid var(--border)", cursor: "pointer" }}
                  ...
                  <td style={{ textAlign: "center", padding: "10px 8px" }}>
                    {badges.includes("AK") && <span style={{ background: "rgba(212,175,55,0.15)", color: "#D4AF37", padding: "2px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600, marginRight: 4 }}>AK</span>}
                    {badges.includes("WP") && <span style={{ background: "rgba(16,185,129,0.15)", color: "#10B981", padding: "2px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600 }}>WP</span>}
                  </td>
