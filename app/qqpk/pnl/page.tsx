...
export default async function QQPKPage({ searchParams }: { searchParams: Promise<{ filter?: string; player?: string }> }) {
  ...
  const filters = { game_name: "QQPK" as const, since_date: startDate, end_date: endDate };
  let summary = getLockAwareSummaryByPlayer(filters) as any[];
  // Phase 1 plumbing: net brut only. Staking C/T P&L arrives in Phase 3.
  let kpis = getLockAwareKPIsWithExtras(filters, "qqpk");
  ...
  const games = (getGames() as any[]).filter((g) => g.name === "QQPK");
  const qqpkGameId = (getDb().prepare(`SELECT id FROM games WHERE name = 'QQPK'`).get() as { id: number } | undefined)?.id;
  ...
  for (const p of players) {
    cashoutsByPlayer[p.id] = qqpkGameId ? getPlayerCashouts(p.id, qqpkGameId) : [];
    gameWalletsByPlayer[p.id] = qqpkGameId ? getPlayerGameWallets(p.id, qqpkGameId) : [];
  }
  ...
  return (
    <>
      <PageHeader
        title="QQPK — P&L"
        subtitle="Dépôts & retraits par joueur — net brut on-chain (staking C/T à venir)"
      />
      {filterPlayerName && (
        <div style={{ padding: "0 28px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="/qqpk/pnl" style={{ fontSize: 11, color: "var(--text-muted)", textDecoration: "none" }}>✕ Retirer le filtre</a>
        </div>
      )}
      <TELEClient
