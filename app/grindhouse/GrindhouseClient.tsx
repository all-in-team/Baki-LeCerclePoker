...
interface Session {
  id: number; player_id: number; game_id: number; session_date: string;
...
export default function GrindhouseClient({ initialSessions, initialDate, players, games }: Props) {
  ...
  async function addSession() {
    ...
    try {
      const res = await fetch("/api/grindhouse-sessions", {
        ...
        body: JSON.stringify({
          player_id: form.player_id, game_id: form.game_id, session_date: date,
          duration_hours: Number(form.duration_hours), net_result_usdt: Number(form.net_result_usdt),
      ...
      if (res.ok) {
        ...
        setForm({ player_id: form.player_id, game_id: form.game_id, duration_hours: "", net_result_usdt: "", notes: "" });
      }
  ...
  async function deleteSession(id: number) {
    ...
    await fetch(`/api/grindhouse-sessions/${id}`, { method: "DELETE" });
  ...
  sessions.forEach(s => {
    const e = byGame.get(s.game_name) ?? { hours: 0, pnl: 0 };
    e.hours += s.duration_hours;
    e.pnl += s.net_result_usdt;
    byGame.set(s.game_name, e);
  });
  ...
  return (
    <div style={{ padding: "0 28px" }}>
      ...
      <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: 10, marginBottom: 24 }}>
        {loading ? (
          <div style={{ padding: 32, textAlign: "center", color: "var(--text-dim)", fontSize: 13 }}>Chargement...</div>
        ) : sessions.length === 0 ? (
          <div style={{ padding: 32, textAlign: "center", color: "var(--text-dim)", fontSize: 13 }}>Aucune session le {date}</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {sessions.map(s => {
                const gb = GAME_BADGES[s.game_name] ?? { short: s.game_name.slice(0, 2), bg: "rgba(156,163,175,0.15)", color: "#9CA3AF" };
                return (
                  <tr key={s.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    <td style={{ padding: "10px 16px", fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{s.player_name}</td>
                    <td style={{ padding: "10px 16px" }}>
                      <span style={{ background: gb.bg, color: gb.color, padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 700 }}>{gb.short}</span>
                    </td>
                    <td style={{ padding: "10px 16px", textAlign: "right", fontSize: 13, color: "var(--text-muted)" }}>{s.duration_hours}h</td>
                    <td style={{ padding: "10px 16px", textAlign: "right", fontSize: 13, fontWeight: 600, color: s.net_result_usdt >= 0 ? "var(--green)" : "#f87171" }}>
                      {s.net_result_usdt >= 0 ? "+" : ""}{fmt(s.net_result_usdt)}
                    </td>
                    <td style={{ padding: "10px 16px", fontSize: 12, color: "var(--text-dim)", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis" }}>{s.notes ?? "—"}</td>
                    <td style={{ padding: "10px 16px", textAlign: "center" }}>
                      <button onClick={() => deleteSession(s.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-dim)", padding: 2, display: "inline-flex" }} onMouseEnter={e => (e.currentTarget.style.color = "#f87171")} onMouseLeave={e => (e.currentTarget.style.color = "var(--text-dim)")}><Trash2 size={14} /></button>
                    </td>
                  </tr>
                );
              })}
      ...
      {sessions.length > 0 && (
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
          <div style={{ padding: "12px 18px", background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 4 }}>Total heures</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text)" }}>{totalHours.toFixed(1)}h</div>
          </div>
          <div style={{ padding: "12px 18px", background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 4 }}>Total P&L</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: totalPnl >= 0 ? "var(--green)" : "#f87171" }}>{totalPnl >= 0 ? "+" : ""}{fmt(totalPnl)} USDT</div>
          </div>
          {[...byGame.entries()].map(([name, v]) => {
            const gb = GAME_BADGES[name] ?? { short: name.slice(0, 2), bg: "rgba(156,163,175,0.15)", color: "#9CA3AF" };
            return (
              <div key={name} style={{ padding: "12px 18px", background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" }}>{v.hours.toFixed(1)}h</span>
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, color: v.pnl >= 0 ? "var(--green)" : "#f87171" }}>{v.pnl >= 0 ? "+" : ""}{fmt(v.pnl)}</div>
              </div>
            );
          })}
        </div>
