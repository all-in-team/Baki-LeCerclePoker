...
import {
  ...
  BadgeCheck, AlertTriangle, ChevronDown, ChevronRight, ArrowDownLeft, ArrowUpRight,
  TrendingUp,
} from "lucide-react";
...
export default function A5SettlementClient({
  ...
  async function openRecap(pid: number, name: string) {
    ...
    try {
      const preview = (await previewAction(pid, ids)) as Preview;
      setRecap({ playerId: pid, playerName: name, ids, preview });
  ...
  return (
    <div style={{ padding: "8px 28px 40px" }}>
      ...
      {walletMeres.length > 0 ? (
        <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: 10, padding: "12px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#4ade80", textTransform: "uppercase", letterSpacing: "0.08em" }}>
            WALLET{walletMeres.length > 1 ? "S" : ""} MÈRE{walletMeres.length > 1 ? "S" : ""} A5POKER
          </div>
          {walletMeres.map(wm => (
      ...
      {/* P&L curve */}
      <NetPnlChart series={netSeries} cardNet={kpis.total_net} currency="USDT" rangeLabel="Lifetime" />
                ...
                {rows.map(r => {
                  ...
                  return (
                    <Fragment key={r.player_id}>
                      <tr style={{ borderBottom: isOpen ? "none" : "1px solid var(--border)", background: avail.length > 0 ? "rgba(245,197,24,0.04)" : undefined }}>
                        <td style={{ ...td, textAlign: "left", fontWeight: 600, color: "var(--text)" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                            <button
                              onClick={() => openRow(r.player_id)}
                              title="Voir / configurer les wallets + régler"
                              style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "3px 6px", borderRadius: 5, cursor: "pointer",
                                background: isOpen ? "rgba(56,189,248,0.15)" : (walletCount === 0 ? "rgba(239,68,68,0.12)" : "var(--bg-base)"),
                                border: `1px solid ${isOpen ? "#38bdf8" : (walletCount === 0 ? "#EF444440" : "var(--border)")}`,
                                color: walletCount === 0 ? "#EF4444" : "var(--text-muted)" }}>
                              <Wallet size={12} />
                              <span style={{ fontSize: 10, fontWeight: 700 }}>{walletCount === 0 ? "0" : walletCount}</span>
                            </button>
                            <button onClick={() => openRow(r.player_id)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: 0, background: "transparent", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "var(--text)" }}>
                              {isOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />}{r.player_name}
                            </button>
                        ...
                        <td style={{ ...td, textAlign: "center" }}>
                          <Btn variant="secondary" onClick={() => openRow(r.player_id)} style={{ fontSize: 11, gap: 5, padding: "6px 10px" }}>
                            <Scale size={13} /> {isOpen ? "Fermer" : "Régler"}
                          </Btn>
                        </td>
                      </tr>
                      {isOpen && (
                        <tr style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-surface)" }}>
                          <td colSpan={COLS} style={{ padding: "16px 20px" }}>
                            ...
                            <div style={{ marginBottom: 18 }}>
                              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}><Wallet size={13} /> Wallets du joueur</div>
                            ...
                            {/* 2. TRANSACTIONS À RÉGLER */}
                            <div style={{ marginBottom: 18, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
                              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}><Scale size={13} color="#F5C518" /> Transactions à régler</div>
                              {avail.length === 0 ? (
                                <div style={{ fontSize: 12, color: "var(--text-dim)" }}>Aucune transaction à régler (tout est settled).</div>
                              ) : (
                                <div>
                                  ...
                                  {avail.map(tx => {
                                    ...
                                    return (
                                      <div key={tx.id} onClick={() => toggleTx(r.player_id, tx.id)} style={{ display: "grid", gridTemplateColumns: "28px 110px 130px 1fr 36px", gap: 10, alignItems: "center", padding: "7px 8px", borderRadius: 6, cursor: "pointer", background: checked ? "rgba(34,197,94,0.08)" : "transparent", borderBottom: "1px solid var(--border)" }}>
                            ...
                            {/* 3. RÈGLEMENTS DU JOUEUR */}
                            {pSettlements.length > 0 && (
                              <div style={{ paddingTop: 14, borderTop: "1px solid var(--border)" }}>
                                <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Règlements du joueur</div>
                                {pSettlements.map(s => {
                                  ...
                                  return (
                                    <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", padding: "8px 10px", borderRadius: 6, marginBottom: 6, background: "var(--bg-base)", border: `1px solid ${isLocked ? "rgba(245,197,24,0.25)" : "var(--border)"}` }}>
                                      <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 700, color: isLocked ? "#F5C518" : "#10B981" }}>{isLocked ? <Lock size={12} /> : <BadgeCheck size={12} />}{isLocked ? "Locked" : "Payé"}</span>
                                      ...
                                      {isLocked ? (
                                        <>
                                          <input value={payHash[s.id] ?? ""} onChange={e => setPayHash(h => ({ ...h, [s.id]: e.target.value }))} placeholder="tx_hash (option.)" spellCheck={false} style={{ width: 140, padding: "5px 8px", borderRadius: 6, fontSize: 11, fontFamily: "monospace", background: "var(--bg-elevated)", color: "var(--text)", border: "1px solid var(--border)", outline: "none" }} />
                                          <button onClick={() => pay(s.id)} disabled={rowBusy === s.id} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "rgba(34,197,94,0.12)", color: "var(--green)", border: "1px solid rgba(34,197,94,0.3)", cursor: "pointer" }}><BadgeCheck size={12} /> Payé</button>
