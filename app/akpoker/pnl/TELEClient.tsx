                  ...
                  {isTxOpen && (
                    <tr style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-surface)" }}>
                      <td colSpan={8} style={{ padding: "12px 20px" }}>
                        {playerTxs.length > 0 && (
                          <div>
                            <div style={{ display: "grid", gridTemplateColumns: "100px 110px 36px 120px 1fr 60px 32px", gap: 12, padding: "6px 0 8px", borderBottom: "1px solid var(--border)", fontSize: 10, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                              <span>Date</span><span>Type</span><span>Src</span><span style={{ textAlign: "right" }}>Montant</span><span>Wallet</span><span style={{ textAlign: "right" }}>Tx</span><span></span>
                            </div>
                            {playerTxs.map(tx => {
                              ...
                              return (
                                <div key={tx.id} style={{ display: "grid", gridTemplateColumns: "100px 110px 36px 120px 1fr 60px 32px", gap: 12, padding: "8px 0", borderBottom: "1px solid var(--border)", alignItems: "center", opacity: beforeStart ? 0.35 : 1 }}>
                                  ...
                                  <span style={{ textAlign: "center" }}>
                                    {tx.source === "manual" && !archived && (
                                      <button onClick={async () => {
                                        if (!confirm(`Supprimer cette transaction manuelle de ${tx.amount.toFixed(2)} ${tx.currency} ? Cette action est irréversible.`)) return;
                                        await fetch(`/api/wallets/${tx.id}`, { method: "DELETE" });
                                        router.refresh();
                                      }} title="Supprimer" style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-dim)", padding: 2, display: "inline-flex" }} onMouseEnter={e => (e.currentTarget.style.color = "#f87171")} onMouseLeave={e => (e.currentTarget.style.color = "var(--text-dim)")}><Trash2 size={12} /></button>
                                    )}
                                  </span>
                                </div>
                              );
