...
// (invariant #2). Mirror of app/a5poker/pnl/actions.ts for KKPOKER.
...
export async function lockAction(playerId: number, txIds: number[], notes?: string) {
  return lockSettlement(kkGameId(), playerId, txIds, notes);
}
...
export async function updateActionPctAction(playerId: number, oldPct: number, newPct: number) {
  ...
  return updateDealActionPct(playerId, ["KKPOKER"], "KKPOKER", oldPct, newPct);
}
