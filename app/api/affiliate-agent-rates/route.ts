...
// Corps : { relationship_id, game_id, agent_pct, start_week: 'YYYY-MM-DD' (lundi) | null (origine),
...
export async function POST(req: NextRequest) {
  ...
  if (!b || !Number.isInteger(b.relationship_id) || !Number.isInteger(b.game_id) || typeof b.agent_pct !== "number"
      || !(b.start_week === null || typeof b.start_week === "string"))
    return NextResponse.json({ ok: false, error: "Corps invalide : { relationship_id, game_id, agent_pct (nombre), start_week (lundi ou null) } attendu." }, { status: 400 });
  const r = setAgentRateOn(getDb(), {
    relationship_id: b.relationship_id, game_id: b.game_id, agent_pct: b.agent_pct, start_week: b.start_week,
