...
import { getInviteLink } from "@/lib/telegram-userbot";
...
export async function POST(req: NextRequest) {
  ...
  if (!/^-100\d{7,14}$/.test(String(chatId))) {
    return NextResponse.json({ error: "Invalid supergroup chat_id format. Expected -100xxxxxxxxxx" }, { status: 400 });
  }

  const result = await getInviteLink(chatId);
