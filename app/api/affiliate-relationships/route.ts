...
export async function GET(req: NextRequest) {
  ...
  const rows = db.prepare(`
    ...
    JOIN games g ON g.id = ar.origin_game_id
    ${where}
...
export async function POST(req: NextRequest) {
  ...
  if (!affiliate_player_id || !referred_player_id || !origin_game_id)
    return NextResponse.json({ error: "affiliate_player_id, referred_player_id, origin_game_id requis" }, { status: 400 });
