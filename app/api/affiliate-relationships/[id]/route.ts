...
export async function GET(_: NextRequest, { params }: Ctx) {
  ...
  const gameRates = db.prepare(
    `SELECT arg.game_id, g.name AS game_name, arg.disclosed_action_pct, arg.disclosed_rakeback_pct, arg.disclosed_insurance_pct, arg.exclude_agency_extras
  ...
  if (Array.isArray(body.game_rates)) {
    const tx = db.transaction(() => {
      ...
      const insert = db.prepare(
        `INSERT INTO affiliate_relationship_games (relationship_id, game_id, disclosed_action_pct, disclosed_rakeback_pct, disclosed_insurance_pct, exclude_agency_extras)
         VALUES (?, ?, ?, ?, ?, ?)`
      );
