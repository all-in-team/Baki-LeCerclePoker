...
export default async function NUTSPKSettlementsPage({ searchParams }: { searchParams: Promise<{ week?: string; player?: string }> }) {
  ...
  return (
    <>
      <PageHeader title="NUTSPK — Settlements" subtitle="Validation hebdomadaire des P&L wallet par joueur" />
      {filterPlayerName && (
        <div style={{ padding: "0 28px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="/nutspk/settlements" style={{ fontSize: 11, color: "var(--text-muted)", textDecoration: "none" }}>✕ Retirer le filtre</a>
        </div>
      )}
      <SettlementsClient
        weekStart={weekStart}
