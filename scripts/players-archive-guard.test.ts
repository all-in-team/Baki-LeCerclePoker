...
db.exec(`
  ...
  CREATE TABLE grindhouse_settlements (player_id INT, status TEXT, period_start TEXT, period_end TEXT);
...
const pGrind = P("Grindhouse"); db.prepare(`INSERT INTO grindhouse_settlements VALUES (?, 'pending', '2026-09-01', '2026-09-07')`).run(pGrind);
eq("grindhouse non payé → ouvert", codes(pGrind), ["grindhouse"]);
const pSess = P("Sessions grindhouse");
db.prepare(`INSERT INTO grindhouse_sessions VALUES (?, '2026-08-03'), (?, '2026-09-10')`).run(pSess, pSess);
db.prepare(`INSERT INTO grindhouse_settlements VALUES (?, 'paid', '2026-08-01', '2026-08-31')`).run(pSess);
...
db.prepare(`INSERT INTO grindhouse_sessions VALUES (?, '2026-08-03')`).run(pSessOk);
db.prepare(`INSERT INTO grindhouse_settlements VALUES (?, 'paid', '2026-08-01', '2026-08-31')`).run(pSessOk);
