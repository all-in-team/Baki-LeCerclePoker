...
11. **Locked weekly settlements are immutable.** Lock anchor = last cashout's tx_datetime in the week window. Players with ≥1 withdrawal auto-lock at compute time (status='settled', locked_by='auto'). Period auto-locks when all player rows reach terminal state (settled/carry_over) — no manual "Lock week" button. `computeWeek` refuses to overwrite locked periods. Late transactions (tx_datetime within a locked week but arriving after lock) belong to the next open week. Once `weekly_settlement_periods.status='locked'`, its `weekly_settlements` rows are immutable.
